...
import './CosmicCafe.css';//importerar CosmicCafe.css
...
import stellar from './Cafe/stellastellartea.jpg';//importerar bild filer

const CosmicCafe = () => {
    const navigate = useNavigate(); //Hjälper till att navigera mellan sidor
    const [slideIndex, setSlideIndex] = useState(1); //Hjälper till att hantera slide index
    
    //Funktion som hanterar slide byte
    const plusSlides = (n) => {
    ...
    //Funktion som visar rätt slide
    const showSlides = () => {
      return (
        <article className="slideshow-container">
          {[...Array(6)].map((_, i) => (
            <figure key={i} className={`menuSlides ${slideIndex === i + 1 ? 'active' : 'none'}`}>
    ...
    //Array av slide bilder
    const slideImages = [darkMatterBrew, calyp, starlight, meteor, celeste, stellar,];
    
    //Array av slide text
    const slideCaptions = ['Dark Matter Brew', 'Calyppuccino', 'Starlight Mocha', 'Meteorite Milkshake', 'Celeste Tea', 'Stella Stellar Tea',];
      ...
      <main className='cosmic-main'>
        ...
        <header>
          <nav className="cosmic-navbar">
            <span className="left">
              <a className="nav-a"><img className="logo" src={cLogo} alt="cosmic-logo" /></a>
              <a className="nav-a nav-txt">Food and Drinks</a>
              <a className="nav-a nav-txt">About</a>
              <a className="nav-a nav-txt">Blog</a>
            </span>
            <span className="right">
              <a className="nav-a order">Order</a>
            </span>
          </nav>
        </header>
